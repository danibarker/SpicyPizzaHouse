...
function updateOrder() {
    ...
    orderHtml +=
        "<div class='row pt-3 justify-content-between' ><button id='addToCart' onclick='confirmation()' class='btn btn-primary'>Add to Cart</button></div>";
    $("#pizzaCost").html(orderHtml);
}
function confirmation() {
    let successHtml = `<div class="alert success" onclick="addToCart()">
...
function showMenu() {
    Object.keys(pizzaToppings).forEach(
        (topping) => (pizzaToppings[topping].quantity = 0)
    );
...
function showPizza() {
    itemType = "pizza";
    $("#order")
        .html(`<button id='goBack' onclick='showMenu()' class='btn btn-secondary'>Back to Menu</button>
        <h2>Make a Pizza</h2>
        <div class="row">
          <div class="col-3">
            <h3>Options</h3>
...
$("#cartlink").click(function () {
    ...
    total *= 1.05;
    let cartHtml =
        '<h2>Cart</h2><div class="container"><div class="row"><div class="col subtotal"><h3>Your Order</h3>';
    for (item of cart) {
        if (typeof item.item == "object") {
            let itemToppings = item.item
                ...
                .reduce(
                    (display, curTopping) =>
                        (display += `${curTopping.toppingType} x ${curTopping.quantity}, `),
                    ""
                );
    ...
    cartHtml += `<div>Delivery cost - $3.00</div><br /><div>Total - $${total.toFixed(
        2
    )}</div>`;
    cartHtml +=
        '</div><div class="col"><h3>Delivery Information</h3>' +
